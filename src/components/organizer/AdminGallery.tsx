...
export function AdminGallery({ eventId }: { eventId: string }) {
  ...
  async function load() {
    const { data } = await supabase.from("gallery")
      .select("id, public_url, moderation, featured, kind, storage_path")
  ...
  async function remove(id: string) {
    ...
    if (row?.storage_path) {
      ...
      if (storageError) { toast.error(storageError.message); setToDelete(null); return; }
    }
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
        {rows.map((r) => (
          <div key={r.id} className="group relative overflow-hidden rounded-xl border bg-muted">
            {r.kind === "video"
              ? <video src={r.public_url} className="aspect-square w-full object-cover" muted preload="metadata" playsInline />
              : <img src={r.public_url} className="aspect-square w-full object-cover" alt="" loading="lazy" />}
            ...
            <div className="absolute inset-x-0 bottom-0 flex justify-center gap-1 bg-gradient-to-t from-black/70 to-transparent p-2 opacity-100 transition md:opacity-0 md:group-hover:opacity-100 md:focus-within:opacity-100">
              <Button size="sm" variant="secondary" className="h-8 w-8 p-0" onClick={() => toggleFeatured(r.id, r.featured)} aria-label={r.featured ? "Quitar de destacadas" : "Destacar"}>
                {r.featured ? <StarOff className="h-3.5 w-3.5" /> : <Star className="h-3.5 w-3.5" />}
              </Button>
              {r.moderation !== "approved" && (
                <Button size="sm" variant="secondary" className="h-8 w-8 p-0" onClick={() => setModeration(r.id, "approved")} aria-label="Aprobar"><Check className="h-3.5 w-3.5" /></Button>
              )}
              {r.moderation !== "rejected" && (
                <Button size="sm" variant="secondary" className="h-8 w-8 p-0" onClick={() => setModeration(r.id, "rejected")} aria-label="Ocultar"><X className="h-3.5 w-3.5" /></Button>
              )}
              <Button size="sm" variant="destructive" className="h-8 w-8 p-0" onClick={() => setToDelete(r.id)} aria-label="Eliminar"><Trash2 className="h-3.5 w-3.5" /></Button>
            </div>
          </div>
        ))}
      </div>
